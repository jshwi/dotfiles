...
def test_mkarchive(nocolorcapsys, repoclone, tmpdir):
    ...
    :param repoclone:        Path to cloned version of this repository
    :param tmpdir:          The temporary directory ``pytest`` fixture.
    """
    _expected = expected.mkarchive(
        tmpdir, repoclone, dotfiles.DATE, dotfiles.TIME
    )
    with unittest.mock.patch.object(sys, "argv", [__name__, repoclone]):
        dotfiles.mkarchive.main()
        out = nocolorcapsys.stdout().splitlines()
        assert out == _expected


def test_mkarchive_file(nocolorcapsys, repoclone, tmpdir):
    ...
    :param tmpdir:          The temporary directory ``pytest`` fixture.
    """
    target = os.path.join(repoclone, "README.rst")
    _expected = expected.mkarchive(
...
def test_mkarchive_no_file(nocolorcapsys, repoclone):
    ...
    :param repoclone:        Path to cloned version of this repository
    """
    target = os.path.join(repoclone, "this_is_not_a_file.txt")
    with unittest.mock.patch.object(sys, "argv", [__name__, target]):
...
def test_mkarchive_relative_file(nocolorcapsys, repoclone, tmpdir):
    """Test the tarring and moving of files with the ``mkarchive``
    ...
    with dotfiles.EnterDir(repoclone):
