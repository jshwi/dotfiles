...
def comment_yaml():
    comments = [
        ...
        "#",
        "# e.g. ``bash`` in ``~/.`` will be linked to ``~/.bash`` and "
        "``settings.json``",
        "#",
...
def main():
    ...
    """
    parser = Parser()
    pre_exist = os.path.isfile(CONFIG)
    ...
    for dot_type in contents:
        for dirpath, obj in contents[dot_type].items():

            if dot_type == "dirs":
                link_dirs(obj, source, dirpath, parser.dry)

            elif dot_type == "files":
                link_files(obj, source, dirpath, parser.dry)
